...
### 6.2 上抬过程中掉 cube：参考走得比真实机械臂快

第一轮日志中，cube 大约在 `retreat_to_safe` 第 76 个控制步掉落。Z 跟踪滞后从约 `70 mm` 增至约
`82 mm`。掉落后完整 6D IK 继续追一个不可达的精确终点，误差无法收敛。

根因包括：

- 同时上抬和内缩给夹持物施加复合侧向载荷；
- 参考速度虽已降低，但执行器/IK 跟踪仍明显滞后；
- 完整 6D 目标对五关节机械臂过紧；
- 旧状态机只看最终误差，掉落后还会继续输出无意义动作。
...
这样至少能区分“还在跟踪”“IK 不可达”和“cube 已经掉了”。
