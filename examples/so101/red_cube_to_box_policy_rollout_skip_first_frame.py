...
def main() -> int:
    ...
    try:
        ...
        def reset_and_skip_first_frame():
            """Reset, discard reset observation 0, then return the first post-step observation."""
            env.reset()

            # Hold the robot at its reset pose while advancing the simulator once.
            # The policy never sees the reset observation.
            ...
            if bool(terminated.any()) or bool(truncated.any()):
                raise RuntimeError("Environment terminated/truncated during reset-frame warmup step")

            print("POLICY_RESET_FRAME_SKIPPED: discarded reset observation 0", flush=True)
            return observations
        ...
        observations = reset_and_skip_first_frame()
